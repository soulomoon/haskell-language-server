...
spec = do
  ...
  describe "apply1" $  do
    itInThread "computes a rule with no dependencies" $ do
      db <- shakeNewDatabaseWithLogger shakeOptions ruleUnit
    ...
    itInThread "tracks direct dependencies" $ do
      db@(ShakeDatabase _ _ theDb) <- shakeNewDatabaseWithLogger shakeOptions $ do
        ruleUnit
        ruleBool
    ...
    itInThread "tracks reverse dependencies" $ do
      db@(ShakeDatabase _ _ Database {..}) <- shakeNewDatabaseWithLogger shakeOptions $ do
        ruleUnit
        ruleBool
      let theKey = Rule @Bool
      res <- shakeRunDatabaseFromRight db $
    ...
    itInThread "rethrows exceptions" $ do
      db <- shakeNewDatabaseWithLogger shakeOptions $ addRule $ \(Rule :: Rule ()) _old _mode -> error "boom"
    ...
    --   cond <- C.newMVar True
    --   count <- C.newMVar 0
    --   (ShakeDatabase _ _ theDb _) <- shakeNewDatabaseWithLogger shakeOptions $ do
    --     ruleUnit
    --     ruleCond cond
    --     ruleSubBranch count
    --     ruleWithCond
