...
- Latest feedback: pushed commit
  `7edc43b2511131e05c170a6937090902a42362f6` restored the `GetModSummary`
  fingerprint shape. Benchmark workflow `25669605154`
  (`https://github.com/soulomoon/haskell-language-server/actions/runs/25669605154`)
  succeeded for the relevant cabal and lsp-types matrix jobs, but operator
  feedback says the benchmark matrix is still different after round 04. Treat
  Milestone 3 as complete evidence, not final parity, and continue to the next
  dependency-ready candidate.
...
`25669605154` on commit `7edc43b2511131e05c170a6937090902a42362f6`
succeeded for the relevant cabal and lsp-types jobs, while operator feedback
says the benchmark matrix is still different.
...
- Direction id: direction-004-semantic-token-rule-parity
  ...
  Why it matters now: Current branch changes semantic tokens from ordinary
    ...
    interest.
  Preconditions: benchmark feedback from milestone 3 is available; satisfied by
    workflow `25669605154` plus operator feedback that differences remain.
