...
- Latest feedback: pushed commit
  `e74533dfc2058ec435e3cc8f2f8bc5fe82fd8c75` restored semantic-token rule
  behavior. Benchmark workflow `25678825121`
  (`https://github.com/soulomoon/haskell-language-server/actions/runs/25678825121`)
  completed successfully for the relevant cabal and lsp-types matrix jobs, but
  operator feedback says the benchmark matrix is still different after round
  05. Treat semantic-token rule parity as complete evidence, not final parity,
  and continue to the next dependency-ready plugin-rule candidate.
...
workflow `25678825121` on commit
`e74533dfc2058ec435e3cc8f2f8bc5fe82fd8c75` succeeded for the relevant cabal
and lsp-types jobs, while operator feedback says the benchmark matrix is still
different. Direction 004 is complete evidence; direction 005 is the next
dependency-ready candidate while plugin-rule suspects remain.
...
- Direction id: direction-005-hlint-and-lower-priority-plugin-parity
  Summary: Restore HLint masking and any remaining lower-priority plugin diffs
    as the next plugin-rule parity candidate.
  Why it matters now: These diffs are lower-probability causes but remain
    branch differences after the semantic-token rule candidate completed
    without final parity.
