...
- Direction id: direction-002-restore-parenttc-fingerprint-rule
  Summary: Restore `FileStore.typecheckParentsAction` to use the separate
    reverse-dependency fingerprint rule rather than depending directly on full
    `GetModuleGraph`.
...
- Direction id: direction-003-restore-modsummary-fingerprint-shape
  ...
  Preconditions: benchmark feedback from milestone 2 is available.
  Parallel hints: Serial only.
